...
const Trnxmodal = ({ action, trnxId, onClose }) => {
	const [errMsg, setErrMsg] = useState("");
	const transactionId = trnxId;
	const mutation = useMutation({
		mutationFn: () => updateTransaction(action, transactionId),
		onSuccess: (data) => {
			console.log(data);
		},
		onError: (err) => {
	...
	const handleSubmit = (e) => {
		e.preventDefault();
		if (!trnxId || !action) {
			setErrMsg("Select an action to perform!");
			return;
		}
		mutation.mutate();
	};
